export function generateScrambles(type: number){
    let randomNumber = 0;
    let notations = [];
...
function generateScramble(count: number, notations: string[], reverseNotations: string[][]){
    let moves: string[] = [];
    ...
    for(let i = 0; i < count; i++){
        ...
        if(i > 0 && move.charAt(0) + "" === moves[moves.length-1].charAt(0) + ""){
            ...
            let rev = Math.floor(Math.random() * 2) ?  true : false;
            if(rev){
                move += "'";
            }
            moves.push(move);
...
export function createScrambleFromArray(moves: string[]){
    let scramble = "";
    moves.forEach(move => {
        scramble += move + " ";
    });
